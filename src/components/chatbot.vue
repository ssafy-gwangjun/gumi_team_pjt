...
const QUERY_TYPE_GUIDE = {
  recommend:
    "각 장소는 이름에서 느껴지는 테마를 짧은 명사구로 끝맺고, 바로 이어서 '~하시는 분들에게 추천해요' 식으로 어떤 사람에게 어울리는지 한 마디 덧붙여줘. 예: '공룡 테마파크. 공룡을 좋아하시는 분들에게 추천해요.', '중화요리 전문점. 든든한 한 끼를 원하시는 분들에게 추천해요.' 처럼. '이름으로 보면', '~일 가능성이 있어요' 같은 헤징 표현은 쓰지 마.",
  schedule:
...
2. 이름에서 유추 가능한 테마는 짧고 단정적인 명사구로 끝맺는다 (예: "중화요리 전문점.", "카페형 음식점.", "강변을 따라 걷기 좋은 공원."). 그 뒤에 이 장소가 어떤 사람에게 어울릴지 "~하시는 분들에게 추천해요" 식으로 한 마디 짧게 덧붙인다 (예: "공룡 테마파크. 공룡을 좋아하시는 분들에게 추천해요."). "~이에요/예요" 문장체는 받침 유무에 따라 표기가 갈려서 문법 오류가 나기 쉬우니 테마를 설명하는 명사구 부분에서는 쓰지 않는다. "이름으로 보면", "~일 가능성이 있어요" 같은 헤징 표현도 쓰지 않는다. 단, "몇 회째", "몇 명 참여" 같은 구체적 수치·사실은 절대 지어내지 않는다.
