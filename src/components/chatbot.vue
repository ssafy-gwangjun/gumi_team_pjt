...
const QUERY_TYPE_GUIDE = {
  recommend:
    "각 장소는 특징 한 줄, 팁이나 추천 포인트 한 줄, 이렇게 두 줄로 나눠서 자유롭게 서술해줘. 정해진 문장 틀을 반복하지 말고 장소마다 다르게 써. 예:\n봄이면 장미가 만개해 산책하기 좋은 공원이에요.\n주말 오전에 가면 한적하게 둘러볼 수 있어요.\n'이름으로 보면', '~일 가능성이 있어요' 같은 헤징 표현은 쓰지 마.",
  schedule:
...
2. 이름에서 자연스럽게 느껴지는 테마나 인상은 자유롭게 설명해도 되지만, 데이터에 없는 구체적 사실(날짜, 요금, 프로그램, 몇 회째·몇 명 참여 같은 수치 등)은 절대 지어내지 않는다. "이름으로 보면", "~일 가능성이 있어요" 같은 헤징 표현은 쓰지 않는다. "~이에요/예요" 문장체는 받침 유무에 따라 표기가 갈려서 오류가 나기 쉬우니 주의해서 쓴다.
3. 이름과 주소만 나열하는 리스트 형태로 답하지 않는다. 반드시 자연스러운 문장/문단으로 설명한다.
4. 장소가 여러 개면 각 장소는 "▶장소명 (주소)" 형식의 한 줄로 시작한다. 번호(1, 2, 3)는 절대 쓰지 않는다. 예: "▶이곡장미공원 (대구광역시 달서구)".
   그 다음 줄부터 그 장소에 대한 설명을 정확히 두 줄로 나눠 쓰되, 문구를 정해진 틀에 맞추지 말고 장소마다 다르게 자유롭게 작성한다.
   - 첫 번째 줄: 그 장소의 특징을 한 문장으로 쓴다 (예: "봄이면 장미가 만개해 산책하기 좋은 공원이에요.").
   - 두 번째 줄: 방문 팁이나 추천 포인트를 한 문장으로 쓴다 (예: "주말 오전에 가면 한적하게 둘러볼 수 있어요.").
   두 줄 모두 매번 같은 표현을 반복하지 말고, 장소의 이름·카테고리에서 느껴지는 인상을 살려 자연스럽게 쓴다. 각 장소 블록(▶줄 + 설명 두 줄) 사이에는 빈 줄을 하나씩 넣어 구분한다.
